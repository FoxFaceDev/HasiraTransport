...
<div class="space-y-5">
    <div class="glass-panel p-6">
        <div class="mb-5">
            ...
            <h2 class="text-2xl font-bold text-slate-900">مێژووی دۆخی خەتەکان</h2>
            <p class="mt-1 text-slate-500">دۆخی هەر بارهەڵگرێک لە کاتی سفرکردنەوەی لیستەکە پارێزراوە.</p>
        </div>
        ...
        <form method="GET" action="{{ route('gatekeeper.history') }}" class="grid grid-cols-1 items-end gap-4 md:grid-cols-4">
            ...
            <label><span class="form-label mb-1 block">گەڕان</span><input type="search" name="search" value="{{ request('search') }}" placeholder="خاوەن، تابلۆ، VIN..." class="glass-input w-full rounded-lg px-4 py-2"></label>
            <button type="submit" class="btn-primary rounded-lg px-5 py-2">پیشاندانی ئەنجام</button>
        </form>
    </div>

    @if($month === now('Asia/Baghdad')->format('Y-m'))
    <div class="space-y-3">
        <div class="flex items-center justify-between px-1">
            <div>
                <h3 class="text-lg font-bold text-slate-900">لیستی ئێستا — هێشتا سفر نەکراوەتەوە</h3>
                <p class="text-sm text-slate-500">ئەم دۆخانە زیندوون و لە سفرکردنەوەی داهاتوودا دەچنە ئەرشیفەوە.</p>
            </div>
            <span class="rounded-full bg-blue-100 px-3 py-1 text-xs font-bold text-blue-700">{{ $currentTankers->count() }} خەت</span>
        </div>
        <div class="glass-panel overflow-x-auto">
            <table class="w-full border-collapse text-right">
                <thead><tr class="border-b border-slate-200 text-slate-500">
                    <th class="px-4 py-3 font-normal">جۆری تۆمار</th><th class="px-4 py-3 font-normal">ڕیزبەندی</th><th class="px-4 py-3 font-normal">خاوەنی خەت</th><th class="px-4 py-3 font-normal">مۆبایل</th><th class="px-4 py-3 font-normal">تابلۆ</th><th class="px-4 py-3 font-normal">جۆر / VIN</th><th class="px-4 py-3 font-normal">دۆخ</th><th class="px-4 py-3 font-normal">بەروار و کاتی دانراو</th><th class="px-4 py-3 font-normal">تێبینی</th>
                </tr></thead>
                <tbody>
                    @forelse($currentTankers as $tanker)
                    ...
                    <tr class="border-b border-slate-100 align-top hover:bg-slate-50">
                        ...
                        <td class="px-4 py-3"><div>{{ $tanker->truck_type }}</div><div class="text-xs text-slate-400">{{ $tanker->vin ?: '-' }}</div></td>
                    ...
                    @empty
                    <tr><td colspan="9" class="px-4 py-10 text-center text-slate-500">هیچ خەتێکی ئێستا بەو فلتەرە نەدۆزرایەوە.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="space-y-3">
        <div class="px-1"><h3 class="text-lg font-bold text-slate-900">تۆمارە ئەرشیفکراوەکان</h3><p class="text-sm text-slate-500">ئەنجامی سفرکردنەوەکانی مانگی {{ $month }}</p></div>
        <div class="glass-panel overflow-x-auto">
        <table class="w-full border-collapse text-right">
            <thead><tr class="border-b border-slate-200 text-slate-500">
                <th class="px-4 py-3 font-normal">کاتی سفرکردنەوە</th><th class="px-4 py-3 font-normal">ڕیزبەندی</th><th class="px-4 py-3 font-normal">خاوەنی خەت</th><th class="px-4 py-3 font-normal">مۆبایل</th><th class="px-4 py-3 font-normal">تابلۆ</th><th class="px-4 py-3 font-normal">جۆر / VIN</th><th class="px-4 py-3 font-normal">دۆخ</th><th class="px-4 py-3 font-normal">بەروار و کاتی دانراو</th><th class="px-4 py-3 font-normal">تێبینی</th>
            </tr></thead>
            <tbody>
                @forelse($items as $item)
                <tr class="border-b border-slate-100 align-top hover:bg-slate-50">
                    <td class="whitespace-nowrap px-4 py-3"><div class="font-semibold">{{ $item->archive->reset_at?->timezone('Asia/Baghdad')->format('Y-m-d H:i') }}</div><div class="text-xs text-slate-400">{{ $item->archive->resetter?->name ?: '-' }}</div></td>
                    ...
                    <td class="px-4 py-3"><div>{{ $item->truck_type }}</div><div class="text-xs text-slate-400">{{ $item->vin ?: '-' }}</div></td>
                ...
                @empty
                <tr><td colspan="9" class="px-4 py-12 text-center text-slate-500">لەو ماوەیەدا هیچ تۆمارێک نەدۆزرایەوە.</td></tr>
                @endforelse
            </tbody>
        </table>
        </div>
