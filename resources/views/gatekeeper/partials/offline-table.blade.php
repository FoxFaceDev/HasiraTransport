                ...
                <tr class="border-b border-white/5 transition-colors" :class="getRowClass(tanker)">
                    ...
                    <td class="py-3 px-4">
                        ...
                        <div x-cloak x-show="!isBlocked(tanker)" class="flex gap-2">
                            <button type="button" @click="getStatus(tanker) === 'green' ? revertStatus(tanker.id) : openScheduleModal(tanker.id, 'green')" :title="getStatus(tanker) === 'green' ? 'بۆ گەڕاندنەوە دووبارە کرتە بکە' : ''" class="rounded-lg border px-3 py-1.5 text-sm transition-colors" :class="getStatus(tanker) === 'green' ? 'bg-emerald-600 border-emerald-600 text-white' : 'bg-emerald-50 border-emerald-200 text-emerald-700 hover:bg-emerald-100'">هاتن</button>
                            <button type="button" @click="getStatus(tanker) === 'yellow' ? revertStatus(tanker.id) : openScheduleModal(tanker.id, 'yellow')" :title="getStatus(tanker) === 'yellow' ? 'بۆ گەڕاندنەوە دووبارە کرتە بکە' : ''" class="rounded-lg border px-3 py-1.5 text-sm transition-colors" :class="getStatus(tanker) === 'yellow' ? 'bg-amber-500 border-amber-500 text-white' : 'bg-amber-50 border-amber-200 text-amber-700 hover:bg-amber-100'">دواخستن</button>
                            <button type="button" @click="getStatus(tanker) === 'red' ? revertStatus(tanker.id) : updateStatus(tanker.id, 'red')" :title="getStatus(tanker) === 'red' ? 'بۆ گەڕاندنەوە دووبارە کرتە بکە' : ''" class="rounded-lg border px-3 py-1.5 text-sm transition-colors" :class="getStatus(tanker) === 'red' ? 'bg-rose-600 border-rose-600 text-white' : 'bg-rose-50 border-rose-200 text-rose-700 hover:bg-rose-100'">نەهاتن</button>
                        </div>
